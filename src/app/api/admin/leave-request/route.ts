...
import { hrUpdateApprovedLeaveRequest } from "@/lib/hr-repo";
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    const updated = await hrUpdateApprovedLeaveRequest({
      requestId,
      fromDate,
      toDate,
      reason,
    ...
    void recordActivity({
      ...
      kind: "leave.hr-edit",
      message: `Updated approved leave request (${requestId.slice(0, 8)}…)`,
    });

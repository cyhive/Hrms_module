...
import { decideWfhRequest } from "@/lib/hr-repo";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const updated = await decideWfhRequest({
      requestId,
    ...
    void recordActivity({
      ...
      kind: "wfh.decision",
      message: `${decisionRaw === "approve" ? "Approved" : "Rejected"} WFH request (${requestId.slice(0, 8)}…)`,
    });
